...
export function AddPhieuNhap() {
  const { toast } = useToast();
  const { register, handleSubmit, reset, setValue } = useForm<PhieuNhap>();
  ...
  const onSubmit = async (formData: PhieuNhap) => {
    const formattedDate = formatDate(new Date(formData.ngayNhap));
    const createData = {
      soLuong: formData.soLuong,
      ngayNhap: formattedDate,
      nhaCungCap: formData.nhaCungCap,
      idNhanVien: formData.nhanVien.idNhanVien || 0,
      idLinhKien: formData.linhKien.idSanPham || 0,
    };
      ...
      <DialogContent className="sm:max-w-[425px]">
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          <div className="grid gap-4 py-4">
            ...
            <div className="grid grid-cols-4 items-center gap-4">
              <Label htmlFor="ngayNhap" className="text-right">
                Ngày nhập
              </Label>
              <Input
                id="ngayNhap"
                type="date"
                {...register("ngayNhap", { required: true })}
                className="col-span-3"
              />
            </div>
            <div className="grid grid-cols-4 items-center gap-4">
              <Label htmlFor="nhaCungCap" className="text-right">
                Nhà cung cấp
              </Label>
              <Input
                id="ghiChu"
                {...register("nhaCungCap", { required: true })}
                className="col-span-3"
              />
            </div>
            <div className="grid grid-cols-4 items-center gap-4">
